...
let rec processTasks problem tasksAndScores filename =
  ...
  | (t, s) :: ts ->
      ...
      let score =
        ...
        float_of_int (s * num) /. float_of_int den
      in
      ...
      let newEntry =
        ...
        | `Assoc fields -> (
            ...
            | _ ->
                failwith
                  "`tests` max_score field in result JSON file is ill-formatted"
            )
        | _ -> failwith "Result JSON file does not have proper formatting"
      in
...
let _ =
  let filename =
    try Unix.getenv "GRADER_SCORES_FILE"
    with Not_found -> failwith "GRADER_SCORES_FILE not defined"
  in
  let problemName =
    try Unix.getenv "GRADER_PROBLEM_NAME"
    with Not_found -> failwith "GRADER_PROBLEM_NAME not defined"
  in
  let tasks =
    try String.split_on_char ' ' (Unix.getenv "GRADER_PROBLEM_TASKS")
    with Not_found -> failwith "GRADER_PROBLEM_TASKS not defined"
  in
  let taskScores =
    let scoreStrings =
      try String.split_on_char ' ' (Unix.getenv "GRADER_TASK_SCORES")
      with Not_found -> failwith "GRADER_TASK_SCORES not defined"
    in
    try List.map int_of_string scoreStrings
    with _ -> failwith "Score in GRADER_TASK_SCORES is not an integer"
  in
